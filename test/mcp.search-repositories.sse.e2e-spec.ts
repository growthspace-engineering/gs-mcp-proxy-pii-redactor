...
describe('MCP Proxy search_repositories via SSE (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Prepare a temp config using SSE transport with github-allow
    ...
    tmpConfigPath = path.join(__dirname, 'config.search.sse.e2e.json');
  ...
  it('exposes search_repositories on github-allow via SSE and executes it', async () => {
      ...
      try {
        ...
        if (!snapshotPayload.count) {
          ...
          console.log(
            'GitHub MCP raw response via SSE (truncated):',
  ...
  // Note: SSE maintains a persistent connection throughout the lifecycle of the client.
  // Multiple operations are inherently tested in the test above (connect, listTools, callTool).
});
